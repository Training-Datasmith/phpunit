...
final class Help
{
    ...
    private int $length_of_longest_option_name = 0;
    private readonly int $columns_available_for_description;
    private bool $has_color;
    public function __construct(?int $width = null, ?bool $with_color = null)
    {
        if ($width === null) {
            $width = (new Console())->get_number_of_columns();
        }
        if ($with_color === null) {
            $this->has_color = (new Console())->has_color_support();
        } else {
            $this->has_color = $with_color;
        }
        foreach ($this->elements() as $options) {
            foreach ($options as $option) {
                if (isset($option['arg'])) {
                    $this->length_of_longest_option_name = max($this->length_of_longest_option_name, strlen($option['arg']));
                }
            }
        }
        $this->columns_available_for_description = $width - $this->length_of_longest_option_name - 4;
    }
    public function generate(): string
    {
        if ($this->has_color) {
            return $this->write_with_color();
        }
        return $this->write_without_color();
    }
    private function write_without_color(): string
    {
        $buffer = '';
        foreach ($this->elements() as $section => $options) {
            ...
            foreach ($options as $option) {
                ...
                if (isset($option['arg'])) {
                    $arg = str_pad($option['arg'], $this->length_of_longest_option_name);
    ...
    private function write_with_color(): string
    {
        $buffer = '';
        foreach ($this->elements() as $section => $options) {
            ...
            foreach ($options as $option) {
                ...
                if (isset($option['arg'])) {
                    $arg = Color::colorize('fg-green', str_pad($option['arg'], $this->length_of_longest_option_name));
                    $arg = preg_replace_callback('/(<[^>]+>)/', static fn(array $matches): string => Color::colorize('fg-cyan', $matches[0]), $arg);
                    $desc = explode(PHP_EOL, wordwrap($option['desc'], $this->columns_available_for_description, PHP_EOL));
                    $buffer .= self::LEFT_MARGIN . $arg . ' ' . $desc[0] . PHP_EOL;
                    for ($i = 1; $i < count($desc); $i++) {
                        $buffer .= str_repeat(' ', $this->length_of_longest_option_name + 3) . $desc[$i] . PHP_EOL;
                    }
                }
            }
            $buffer .= PHP_EOL;
