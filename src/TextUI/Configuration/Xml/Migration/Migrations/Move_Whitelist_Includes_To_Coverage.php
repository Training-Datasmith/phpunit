 ...
 */
final readonly class Move_Whitelist_Includes_To_Coverage implements Migration
{
    /**
    ...
    public function migrate(Dom_Document $document): void
    {
        ...
        $include = $document->create_element('include');
        $coverage->append_child($include);
        foreach (Snapshot_Node_List::from_node_list($whitelist->child_nodes) as $child) {
            if (!$child instanceof Dom_Element) {
                continue;
            }
            if (!($child->node_name === 'directory' || $child->node_name === 'file')) {
                continue;
            }
            $include->append_child($child);
        }
    }
}